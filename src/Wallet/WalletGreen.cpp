...
void parseAddressString(const std::string& string, const MevaCoin::Currency& currency, MevaCoin::AccountPublicAddress& address) {
  if (!currency.parseAccountAddressString(string, address)) {
    throw std::system_error(make_error_code(MevaCoin::error::BAD_ADDRESS));
  }
}

void validateAddresses(const std::vector<std::string>& addresses, const MevaCoin::Currency& currency) {
...
void validateOrders(const std::vector<WalletOrder>& orders, const MevaCoin::Currency& currency) {
  for (const auto& order: orders) {
    if (!MevaCoin::validateAddress(order.address, currency)) {
      throw std::system_error(make_error_code(MevaCoin::error::BAD_ADDRESS));
    }

    if (order.amount >= static_cast<uint64_t>(std::numeric_limits<int64_t>::max())) {
      throw std::system_error(make_error_code(MevaCoin::error::WRONG_AMOUNT),
        "Order amount must not exceed " + std::to_string(std::numeric_limits<int64_t>::max()));
    }
  }
}

uint64_t countNeededMoney(const std::vector<MevaCoin::WalletTransfer>& destinations, uint64_t fee) {
...
void checkIfEnoughMixins(std::vector<MevaCoin::COMMAND_RPC_GET_RANDOM_OUTPUTS_FOR_AMOUNTS::outs_for_amount>& mixinResult, uint64_t mixIn) {
...
MevaCoin::AccountPublicAddress parseAccountAddressString(const std::string& addressString, const MevaCoin::Currency& currency) {
...
void WalletGreen::validateTransactionParameters(const TransactionParameters& transactionParameters) {
  ...
  if (transactionParameters.changeDestination.empty()) {
    ...
    if (!MevaCoin::validateAddress(transactionParameters.changeDestination, m_currency)) {
      throw std::system_error(make_error_code(MevaCoin::error::BAD_ADDRESS), "Wrong change address");
    }
...
bool WalletGreen::isMyAddress(const std::string& addressString) const {
  MevaCoin::AccountPublicAddress address = parseAccountAddressString(addressString, m_currency);
