...
void wallet_rpc_server::processRequest(const MevaCoin::HttpRequest& request, MevaCoin::HttpResponse& response) {
...
bool wallet_rpc_server::on_transfer(const wallet_rpc::COMMAND_RPC_TRANSFER::request& req, wallet_rpc::COMMAND_RPC_TRANSFER::response& res) {
  ...
  try {
    ...
    MevaCoin::TransactionId tx = m_wallet.sendTransaction(transfers, req.fee, extraString, req.mixin, req.unlock_time);
