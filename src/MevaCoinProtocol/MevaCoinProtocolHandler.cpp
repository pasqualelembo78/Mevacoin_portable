...
bool MevaCoinProtocolHandler::process_payload_sync_data(const CORE_SYNC_DATA& hshd, MevaCoinConnectionContext& context, bool is_inital) {
  if (context.m_state == MevaCoinConnectionContext::state_befor_handshake && !is_inital)
    return true;
...
int MevaCoinProtocolHandler::handleCommand(bool is_notify, int command, const BinaryArray& in, BinaryArray& out, MevaCoinConnectionContext& ctx, bool& handled) {
  ...
  switch (command) {
    ...
    HANDLE_NOTIFY(NOTIFY_REQUEST_TX_POOL, &MevaCoinProtocolHandler::handleRequestTxPool)
...
int MevaCoinProtocolHandler::processObjects(MevaCoinConnectionContext& context, const std::vector<block_complete_entry>& blocks) {
