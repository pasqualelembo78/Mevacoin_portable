...
Crypto::Hash BlockchainMonitor::requestLastBlockHash() {
  ...
  try {
    MevaCoin::HttpClient client(m_dispatcher, m_daemonHost, m_daemonPort);

    MevaCoin::COMMAND_RPC_GET_LAST_BLOCK_HEADER::request request;
    MevaCoin::COMMAND_RPC_GET_LAST_BLOCK_HEADER::response response;

    System::EventLock lk(m_httpEvent);
    MevaCoin::JsonRpc::invokeJsonRpcCommand(client, "getlastblockheader", request, response);
