...
#ifdef _WIN32
const std::string DAEMON_FILENAME = std::string(MEVACOIN_NAME) + "d.exe";
#else
const std::string DAEMON_FILENAME = std::string(MEVACOIN_NAME) + "d";
#endif
...
void BaseFunctionalTests::startNode(size_t index) {
  ...
  config
    ...
    << "log-file=test_" << MEVACOIN_NAME << "d_" << index << ".log" << std::endl;
...
#if defined WIN32
  std::string commandLine = "start /MIN \"" + std::string(MEVACOIN_NAME) + "d" + std::to_string(index) + "\" \"" + daemonPath.string() +
  ...
  if (pid == 0) {
    ...
    if (execl(pathToDaemon.c_str(), (std::string(MEVACOIN_NAME) + "d").c_str(), "--testnet", dataDir.c_str(), "--config-file=daemon.conf", NULL) == -1) {
...
bool BaseFunctionalTests::makeWallet(std::unique_ptr<MevaCoin::IWalletLegacy> & wallet, std::unique_ptr<MevaCoin::INode>& node, const std::string& password) {
  ...
  wallet = std::unique_ptr<MevaCoin::IWalletLegacy>(new MevaCoin::WalletLegacy(m_currency, *node));
