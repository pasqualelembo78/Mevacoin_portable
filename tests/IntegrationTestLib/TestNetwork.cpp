...
#ifdef _WIN32
const std::string daemonExec = std::string(MevaCoin::MEVACOIN_NAME) + "d.exe";
#else
const std::string daemonExec = std::string(MevaCoin::MEVACOIN_NAME) + "d";
#endif
...
TestNodeConfiguration TestNetworkBuilder::buildNodeConfiguration(size_t index) {
  ...
  cfg.testnet = testnet;
  cfg.logFile = std::string("test_") + MevaCoin::MEVACOIN_NAME + "d" + std::to_string(index) + ".log";
