...
bool constructMinerTxManually(const MevaCoin::Currency& currency, uint32_t height, uint64_t alreadyGeneratedCoins,
  const MevaCoin::AccountPublicAddress& minerAddress, MevaCoin::Transaction& tx, uint64_t fee,
  MevaCoin::KeyPair* pTxKey = 0);
bool constructMinerTxBySize(const MevaCoin::Currency& currency, MevaCoin::Transaction& baseTransaction, uint32_t height,
